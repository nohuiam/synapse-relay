...
import { addRule, updateRule, removeRule, listRules, getRule } from '../relay/rule-engine.js';
...
export const CONFIGURE_RELAY_TOOL: Tool = {
...
export function handleConfigureRelay(args: unknown): ConfigureRelayOutput {
  ...
  switch (input.action) {
    case 'add': {
      ...
      const ruleId = addRule(input.signal_pattern, input.relay_to, {
        ...
        transform: input.transform as Record<string, unknown> | undefined,
