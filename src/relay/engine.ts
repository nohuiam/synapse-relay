 ...
 */
export function initRelayEngine(cfg: InterlockConfig, socket: dgram.Socket): void {
  config = cfg;
  udpSocket = socket;
}
 ...
 */
export async function relaySignal(request: RelayRequest): Promise<RelayResult> {
  ...
  for (const rule of rules) {
    if (rule.transform) {
      const transform = JSON.parse(rule.transform) as Record<string, unknown>;
  ...
  const results = await Promise.allSettled(
    request.target_servers.map(target =>
      sendToTarget(target, request.signal_type, payload, request.priority)
    )
  );
 ...
 */
export async function multicastSignal(
  signalType: number,
  sourceServer: string,
  payload: Record<string, unknown>,
