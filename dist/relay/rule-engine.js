 ...
 */
export function matchRules(signalType, sourceServer) {
 ...
 */
export function addRule(signalPattern, relayTo, options = {}) {
    const db = getDatabase();
...
export function listRules() {
    ...
    return rules.map(r => ({
        ...
        relay_to: JSON.parse(r.relay_to),
        transform: r.transform ? JSON.parse(r.transform) : null,
        priority: r.priority,
        enabled: r.enabled === 1,
 ...
 */
export function getAutoRelayTargets(signalType, sourceServer) {
    const rules = matchRules(signalType, sourceServer);
    const targets = new Set();
    for (const rule of rules) {
        const relayTo = JSON.parse(rule.relay_to);
