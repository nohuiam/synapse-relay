 ...
 */
export function startStatsAggregation(intervalMs = 3600000) {
...
export function aggregateHourlyStats() {
    ...
    for (const relay of relays) {
        const targets = JSON.parse(relay.target_servers);
        const reached = relay.targets_reached ? JSON.parse(relay.targets_reached) : [];
        const failed = relay.targets_failed ? JSON.parse(relay.targets_failed) : [];
 ...
 */
export function getRelayStatistics(since, until, groupBy, includeFailures = true) {
    const db = getDatabase();
 ...
 */
export function cleanupOldStats(retentionDays) {
    const db = getDatabase();
