...
export const GET_RELAY_STATS_TOOL = {
...
export function handleGetRelayStats(args) {
    const input = GET_RELAY_STATS_SCHEMA.parse(args || {});
    // Default to last 24 hours if no time range specified
    const since = input.since ?? Date.now() - (24 * 60 * 60 * 1000);
    const until = input.until;
    const stats = getRelayStatistics(since, until, input.group_by, input.include_failures ?? true);
    return {
...
//# sourceMappingURL=get-relay-stats.js.map